  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      if (selectedBrands.length > 0) {
        filters.brand = selectedBrands.join(","); // ✅ IMPORTANT
      }

      if (selectedCategories.length > 0) {
        filters.category = selectedCategories.join(","); // ✅ IMPORTANT
      }
      ...
      <div className={`w-full md:w-[280px] p-6 border-r border-gray-800 ${sidebarOpen ? 'block' : 'hidden'} md:block`}>
        ...
        <ul className="text-[#A9AFC3] mt-4 space-y-2 text-sm">
          {['Latest', 'Featured', 'Top Rated', 'Price (Lowest First)', 'Price (Highest First)'].map((option) => (
            <li
              key={option}
              className={`cursor-pointer hover:text-red-500 ${sortBy === option ? 'text-red-500' : ''}`}
        ...
        <p className="text-[#A9AFC3] mt-2">₹{priceRange.toLocaleString('en-IN')}</p>
